...
class DNSRIntegration:
    ...
    def _preprocess_pulse_data(self, pulse_data: np.ndarray) -> List[np.ndarray]:
        ...
        try:
            # 小波变换（WCNN格式）
            coeffs = pywt.wavedec(pulse_data, 'haar', level=3)
            
            # 重塑为模型输入格式
            cA3 = coeffs[0].reshape(1, -1, 1)
            cD3 = coeffs[1].reshape(1, -1, 1)
            cD2 = coeffs[2].reshape(1, -1, 1)
            cD1 = coeffs[3].reshape(1, -1, 1)
